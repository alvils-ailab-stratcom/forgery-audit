...
Folder-based Resemble AI deepfake analysis. Deliverable: one official-document Latvian report per image, video or audio file, as Markdown and PDF with identical content (`results/<case>/reports/<file>.lv.md` and `.lv.pdf`), plus the cover `atzinums.lv.md`/`.pdf`. English technical analysis, provider answers and complete raw API response bodies remain beside them. Each report is fully Latvian and has a header table (document number, date, analyst named via `ANALYST`, file, SHA-256, format), then 1. Atzinums (half-page conclusion answering the seven examination questions in continuous prose), 2. checks performed with their results and Resemble's own scores (detector per modality, watermark Perth/SynthID, C2PA, reverse image search, audio source tracing, Intelligence assessments, abnormalities, transcription, question count, EXIF or container metadata), 3. numbered references (reverse-image-search articles with resolved URLs and translated reasons, documentation of each method), 4. a score legend, and a signature block. No images; the questions themselves are not printed.
...
make analyze DATA=data OUTPUT=results/audit ANALYST="Alvils Sture"  # the analyst's name is required once per case
...
- `conclusion.lv.txt`: reviewed Latvian prose written by the agent; replaces the automatic text in the report.
- `intelligence.lv.json`: the agent's Latvian translations of provider free text (alterations, fraud reasoning, abnormalities, reverse-search reasons); untranslated free text is not printed.
- `../analyst.json`: the analyst named with `--analyst`, reused by later offline runs.
- `report.lv.json`: structured Latvian narrative with the seven answers, for downstream tooling.
- `../reports/<file>.lv.md` + `.lv.pdf` and `../atzinums.lv.md` + `.pdf`: the delivered documents, regenerated by every run; the PDF is rendered from the Markdown.
